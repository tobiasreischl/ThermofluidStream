...
inlets and outlets the volume is connected to.
 ...
 h_start=Medium.h_default
  ...
  parameter Medium.MassFlowRate m_flow_assert(max=0) = -
  ...
  parameter Boolean usePreferredMediumStates=false
    ...
    annotation (Dialog(tab="Advanced"));
  parameter Medium.MassFlowRate m_flow_reg=dropOfCommons.m_flow_reg
...
<p>To complete the partialTank, equations for total tank volume V, centreOfMass, staticHeadInlets, staticHeadOutlets, staticHeadRears and staticHeadFores needs to be provided. </p>
...
<p>This Volume is the parent class for Accumulator and Receiver models that separate the two phases and are able to output gas, liquid or two-phase medium, depending on its liquid level and the height of the outlet. Numerical stiffness is handled as in VolumeFlex.</p>
