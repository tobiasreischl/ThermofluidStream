...
partial model PartialNTU "Partial heat exchanger model using the epsilon-NTU method"
  ...
  parameter Modelica.Units.SI.Time TC=5 "Heat exchanger time constant (see documentation for better estimation)";
...
<ul>
<li>if the mass flow on both sides of the heat exchanger is zero, no heat is transferred </li>
</ul>
<p><b><span style=\"font-size: 8pt;\">Heat exchanger time constant</span></b> </p>
<p>The model approximates the transient behavior of the heat exchanger with a first-order ordinary differential equation.</p><p>Based on the conservation of energy one can derive that its time constant <code>TC</code> can be approximated by the ratio of thermal inertia (wall + fluid) <code>dU / dT</code> to the enthalpy flow rate &quot;inertia&quot; <code>dH_flow / dT</code>: </p>
<p><code>TC ~ (m_HEX * c_HEX + m_FluidA * c_FluidA + m_FluidB * c_FluidB) / (m_flow_FluidA * c_FluidA + m_flow_FluidB * c_FluidB)</code> , </p>
<p>where <code>m</code> is the mass, <code>m_flow</code> is the mass flow rate, <code>c</code> is the specific heat capacity, <code>U</code> is the internal energy, <code>T</code> is the temperature and <code>H_flow = m_flow*h</code> is the enthalpy flow rate. </p>
<p>For example a heat exchanger with a mass of <code>10 kg</code> and mass flow rates of <code>0.5 kg/s</code> of air on both sides has a time constant in the magnitude of about <code>10 s</code>. The default time constant can also lead to a stiff system, and thereby increase simulation time. </p>
<p style=\"margin-left: 30px;\">Air-Air: <code>TC = 5&hellip;20 s</code></p>
<p style=\"margin-left: 30px;\">Water-Water (compact): <code>TC = 1&hellip;5 s</code></p>
<p style=\"margin-left: 30px;\">bigger system / low fluent massflow: <code>TC = 20&hellip;60 s</code> or even more</p>
<p>Note that the time constant also avoids algebraic loops and may also be beneficial from a numerical point of view. </p>
</html>"));
end PartialNTU;
